...
module i2s2audio (
	input wire resetn,
	input wire cpuclock,	// CPU clock
    input wire audioclock,	// 22.591MHz master clock

	input wire abempty,
	input wire abvalid,
	output logic audiore = 1'b0,
...
always@(posedge audioclock) begin
	if (resetn == 1'b0) begin
		...
		audiore <= 1'b0;
		if ((~abempty) & (count==9'h0ff)) begin // Trigger new sample copy just before we select L channel again out of the LR pair
			audiore <= 1'b1;
		end
		if (abvalid) begin
			tx_data_r <= {8'd0, leftrightchannels[31:16], 8'd0};
			tx_data_l <= {8'd0, leftrightchannels[15:0], 8'd0}; // Unfortunately this breaks absolute silence 
		end
	end
end
